...
if ( ! function_exists('isUserLogged')) {
    ...
    {
//        \Log::info(varDump(Auth::user(), ' -1 isUserLogged Auth::user()::'));
        return Auth::user();
    }
} // if ( ! function_exists('isUserLogged')) {

if ( ! function_exists('checkLoggedUserAccess')) {
    function checkLoggedUserAccess(array $checkAccess, $loggedUser = null)
    {
        if (empty($loggedUser)) {
            $loggedUser = \Auth::user();
        };
        foreach ($loggedUser->permissions as $nextPermission) {
            foreach ($nextPermission->roles as $nextPermissionRole) {
                if (in_array($nextPermissionRole->id, $checkAccess)) {
                    return true;
                }
            }
        }
...
if ( ! function_exists('getLoggedUserDisplayName')) {
    function getLoggedUserDisplayName()
    {
        ...
        $loggedUser = Auth::user();
        if ($loggedUser->first_name or $loggedUser->last_name) {
            return (isDeveloperComp() ? $loggedUser->id . ' : ' : '') . trim($loggedUser->first_name . ' ' . $loggedUser->last_name);
        }
        $ret = $loggedUser->first_name;
        if ( ! empty($ret)) {
